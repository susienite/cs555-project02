...
def check_birth_before_marriage(indi_data, fam_data, errors):
    for fam in fam_data:
        ...
        if(husb_data['Birthday'] == 'NA' or wife_data['Birthday'] == 'NA' or fam['Married'] == 'NA'): continue
        ...
        if(compareDates(wife_data['Birthday'], fam['Married']) != -1):
            errors.append(f'ERROR: INDIVIDUAL: US08: Birth date ({wife_data["Birthday"]}) of {wife_data["Name"]} ({wife_data["ID"]}) occurs on the same day or after her marriage date ({fam["Married"]})')
